...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'mu' ) ) {
		opts.mu = options.mu;
		if ( !isNumber( opts.mu ) ) {
			return new TypeError( 'mgf()::invalid option. `mu` parameter must be a number primitive. Option: `' + opts.mu + '`.' );
		}
	}
	if ( options.hasOwnProperty( 's' ) ) {
		opts.s = options.s;
		if ( !isPositive( opts.s ) ) {
			return new TypeError( 'mgf()::invalid option. `s` parameter must be a positive number. Option: `' + opts.s + '`.' );
		}
	}
